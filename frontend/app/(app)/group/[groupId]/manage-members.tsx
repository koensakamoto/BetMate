...
export default function ManageMembers() {
  ...
  const handleMemberPress = (member: GroupMemberResponse) => {
    ...
    actions.push({ text: 'View Profile', onPress: () => router.push(`/(app)/profile/${member.id}`) });
    ...
    // Owner can transfer ownership to any active member
    if (isOwner) {
  ...
  const handleTransferOwnership = async (member: GroupMemberResponse) => {
    Alert.alert(
      'Transfer Ownership',
      `Are you sure you want to transfer ownership of this group to ${getDisplayName(member)}?\n\nThis action cannot be undone. You will remain as an admin but will no longer be the owner.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Transfer',
          ...
          onPress: async () => {
            try {
              const numericGroupId = Array.isArray(groupId) ? parseInt(groupId[0]) : parseInt(groupId as string);
              await groupService.transferOwnership(numericGroupId, member.id);

              // Update local state to reflect new owner
              setGroupData(prev => prev ? { ...prev, ownerUsername: member.username } : prev);
    ...
    <View style={{ flex: 1, backgroundColor: '#0a0a0f' }}>
      ...
      <ScrollView style={{ flex: 1, paddingHorizontal: 20 }}>
        {filteredMembers.map((member) => {
          ...
          const canShowActionsForMember = isOwner || (canManageMembers && member.id !== user?.id);

          return (
            <TouchableOpacity
              key={member.id}
              onPress={handleCardPress}
              onLongPress={() => canShowActionsForMember && handleMemberPress(member)}
              activeOpacity={0.7}
              style={{
            ...
            >
              ...
              {/* More options button for owner/admins */}
              {canShowActionsForMember && (
                <TouchableOpacity
