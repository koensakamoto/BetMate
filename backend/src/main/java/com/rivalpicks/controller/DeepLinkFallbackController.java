...
/**
 * Fallback controller for deep links when Universal/App Links don't trigger.
 *
 * When Universal Links (iOS) or App Links (Android) are configured and the app IS installed,
 * the OS should intercept the URL and open the app directly.
 *
 * This controller handles requests when:
 * - Universal/App Links don't trigger (caching issues, etc.)
 * - The app is NOT installed on the device
 * - The user is on a desktop browser
 *
 * It serves a smart redirect page that:
 * 1. Tries to open the app using the custom URL scheme (rivalpicks://)
...
public class DeepLinkFallbackController {
    ...
    private ResponseEntity<String> handleFallback(HttpServletRequest request) {
        // Get the path and query to construct the deep link
        String path = request.getRequestURI();
        String queryString = request.getQueryString();
        String fullPath = path + (queryString != null ? "?" + queryString : "");

        // Build the custom scheme URL (rivalpicks://auth/verify-email-change?token=xxx)
        String appSchemeUrl = "rivalpicks:/" + fullPath;
        ...
        // Serve a smart page that tries custom scheme first, then falls back to store
    ...
    private String buildSmartRedirectPage(String appSchemeUrl, String storeUrl) {
        return """
            ...
            <head>
                ...
                <title>Opening RivalPicks...</title>
                <style>
